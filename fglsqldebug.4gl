...
FUNCTION init_database(filename, force_reload)
   ...
   #LET db = SFMT("tmpdb+driver='dbmsqt',source='%1'",escape_backslashes(tmpfile))
    LET db = SFMT("ds+driver='dbmoraB2x',source='%1'",'topprod')
    DISPLAY db
    #DATABASE SYSTEM
   #CONNECT TO db
    CONNECT TO l_source USER l_user USING l_passwd
...
FUNCTION load_file(filename, force_reload)
   ...
   #EXECUTE IMMEDIATE "VACUUM"
    ...
    DISPLAY SFMT("=== RUN %1","purge recyclebin")
    ...
    CATCH
        ...
        DISPLAY SFMT("=== ERROR %1 %2","open file",filename)
        RETURN FALSE
    END TRY
